...
function MarketPresence({ data }: { data: Json }) {
  ...
  if (data.packageId) return <article className="record-card"><div className="record-top"><div><p className="micro-label">service package draft</p><h2>{String(data.title)}</h2></div><CapabilityBadge value={String(data.capability)} /></div><p>{String(data.description)}</p><div className="tier-grid">{arrayOf(data.tiers).map((tier, index) => <div className="tier" key={index}><strong>{String(tier.name)}</strong><span>{formatMoney(Number(tier.priceMinor), String(data.currency))}</span><small>{String(tier.deliveryDays)} days</small></div>)}</div></article>;
